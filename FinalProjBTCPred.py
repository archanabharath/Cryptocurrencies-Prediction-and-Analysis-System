...
About : This is the module that builds LSTM prediction model for predicting future prices of bitcoin data
...
def btc_daily_pred_lstm(df_close_float, days_count):
    ...
    # calculating RMSE for evaluating the model built

    print("-----------------------Bitcoin Prediction model Accuracy ---------------------")
    ...
    plt.title("Bitcoin Close Price Trend Graph")
    plt.plot(trainPredictPlot, label = 'Train Predictions')
    plt.plot(testPredictPlot, label = 'Test Predictions')
    plt.legend()
    btc_trend_img_name = "BTC-trend-plot.png"
    plt.savefig(btc_trend_img_name)
    plt.close()

    return round(futurePredict[0][0], 2), round(prediction_tomo[0][0], 2),round(prediction_that_date[0][0],2), btc_trend_img_name
