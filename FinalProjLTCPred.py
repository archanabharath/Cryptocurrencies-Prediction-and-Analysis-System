...
File : FinalProjLTCPred.py
About : This is the module that builds LSTM prediction model for predicting future prices of litecoin data
...
def create_test_train(df_scaled_1):
    data_t = []
    data_t_next = []
    for i in range(len(df_scaled_1) - 1):
        data_t.append(df_scaled_1[i])
        data_t_next.append(df_scaled_1[i + 1])
...
def ltc_daily_pred_lstm(df_close_float, days_count):
    ...
    model.fit(train_t, train_nextT, epochs=300, batch_size=33, verbose=1)  # 300,33 is best fit so far
    ...
    # calculating RMSE for evaluating the performance of the model

    print("-----------------------Litecoin Prediction model Accuracy ---------------------")
    ...
    plt.xlabel("Time")
    plt.ylabel("LTC Close Price")
    plt.title("Litecoin Close Price Trend Graph")
    plt.plot(trainPredictPlot, label = 'Train Predictions')
    plt.plot(testPredictPlot, label = 'Test Predictions')
    plt.legend()
    ltc_trend_img_name = "LTC-trend-plot.png"
    plt.savefig(ltc_trend_img_name)
    plt.close()

    return round(futurePredict[0][0], 2), round(prediction_tomo[0][0], 2) ,round(prediction_that_date[0][0],2), ltc_trend_img_name
